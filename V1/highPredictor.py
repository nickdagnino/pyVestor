...
X = data[['Open', 'Low', 'Close', 'Adj Close', 'Volume']].values
y = data['High'].values
...
new_high = data[['Open', 'Low', 'Close', 'Adj Close', 'Volume']].values

new_high = StandardScaler().fit_transform(new_high)

next_high = np.dot(X, weights) + bias

print(f"The predicted next value is: {next_high[len(next_high)-1]}")